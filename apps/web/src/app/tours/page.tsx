...
function ToursContent() {
  ...
  const handleToggleStatus = async (tour: NonNullable<typeof tours>[number]) => {
    if (tour.status === "active") {
      ...
      await activateTour({ id: tour._id });
    }
  };

  const handleDuplicate = async (id: Id<"tours">) => {
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold">Product Tours</h1>
          <p className="text-gray-500">
            Guide users through your product with interactive walkthroughs
          </p>
        </div>
            ...
            <tbody className="divide-y">
              {filteredTours?.map((tour: NonNullable<typeof tours>[number]) => (
                <tr key={tour._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getStatusBadge(tour.status)}`}
                    >
                      {tour.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-1">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleDuplicate(tour._id)}
                        title="Duplicate"
                      >
