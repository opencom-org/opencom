...
async function evaluateSeriesReadiness(
  ...
  for (const connection of connections) {
    if (!blockIds.has(connection.fromBlockId as string) || !blockIds.has(connection.toBlockId as string)) {
      blockers.push(
        createReadinessIssue(
  ...
  for (const block of blocks) {
    ...
    if (block.type === "rule") {
      ...
      const defaultCount = conditioned.filter((connection) => connection.condition === "default").length;
    ...
    if (block.type === "wait") {
      ...
      if (config.waitType === "duration") {
        if (!Number.isFinite(config.waitDuration) || Number(config.waitDuration) <= 0 || !config.waitUnit) {
          blockers.push(
            createReadinessIssue(
    ...
    if ((block.type === "chat" || block.type === "post" || block.type === "carousel") && !hasTextContent(config.body)) {
      warnings.push(
        createReadinessIssue(
